...
registerBlockType( 'udemy/present-event', {
    title:                              __( 'Present Events', 'event' ),
    description:                        __(
        'Yields present events.',
        'event'
    ),
    ...
    edit: ( props ) => {
        ...
        const {
            ...
            useEffect(() => {
                const events = apiFetch({ url: "http://localhost:10018/wp-json/wp/v2/event?orderby=date",
                }).then((events) => {
                console.log(events);
                props.setAttributes({
            ...
            const author_map = post_authors_name.reduce(function (author_map, field, index) {
            ...
            return [
                <InspectorControls>
                    ...
                    <PanelBody title={__("Basics", "event")}>
                        ...
                        <p>
                            {__(
                            "Configure the contents of your block here.",
                            "recipe"
                            )}
                        </p>
                        </PanelRow>
                ...
                <div className={props.className}>
                    ...
                    {props.attributes.recent_events ? (
                        (props.events = props.attributes.recent_events.map(
                        (item) => {
                            ...
                            { displayPostContent &&
								displayPostContentRadio === 'excerpt' && (
									<div className="wp-block-latest-posts__post-excerpt">
										{ stripHtml(excerptCount( item.excerpt.rendered, excerptLength )) }
									</div>
								) }
                            { displayPostContent &&
